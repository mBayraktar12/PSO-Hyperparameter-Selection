...
def pso_hyperparameter_optimization(estimator, X_train, X_test, y_train, y_test, num_particles, num_iterations, c1, c2, num_jobs=-1):
    """
    ...
    for _ in range(num_iterations):
        fitness = Parallel(n_jobs=num_jobs)(
            delayed(evaluate_fitness)(estimator, X_train, X_test, y_train, y_test, particle)
            for particle in population
        )

        for j, particle in enumerate(population):
            if fitness[j] > evaluate_fitness(estimator, X_train, X_test, y_train, y_test, best_position[j]):
...
def evaluate_fitness(estimator, X_train, X_test, y_train, y_test, hyperparameters):
    """
